...
static uint8_t rows[] = {0xFE, 0xFD, 0xFB, 0xF7};
/* USER CODE END PTD */
...
private:
    ...
    uint8_t key = 0;
    uint8_t input;
    uint8_t uncheckedInput;
    bool isExistsUnread;
    std::unordered_map<uint8_t, char> keysTranslator = {{0x11, '1'},
                                                        {0x21,
                                                               '2'},
                                                        {0x41, '3'},
                                                        {0x12, '4'},
    ...
    void push() {
        ...
        if (it != keysTranslator.end()) {
            uint8_t translatedKey = it->second;
            if (translatedKey != input && translatedKey == uncheckedInput) {
                input = translatedKey;
                isExistsUnread = true;
            } else {
...
void printModeSwitched() {
    Writer::printChar('\n');
    Writer::printString("Mode switched. Current: ");
    Writer::printString(GAME_MODE ? "GAME" : "TEST");
    Writer::printChar('\n');
}
