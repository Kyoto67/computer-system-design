...
public:
	...
	void ensureForDataIsSent() {
		if (current == UartDriver::INT && !output.isEmpty()) {
			InterruptGuard guard { };
			char *c = output.front();
			if (HAL_OK == HAL_UART_Transmit_IT(&huart6, (uint8_t*) c, 1)) {
				output.pop();
			}
		}
	}
...
public:
	bool askNewCode(char toFill[], uint8_t len, Session currentSession) {
		...
		Printer::printString(repeatNewPasscodeMessage,
				sizeof(repeatNewPasscodeMessage) / sizeof(char));
		Printer::printString(tmp, sizeof(tmp) / sizeof(char));
		char confirmingMessage[] = "\nConfirm? (y/n): ";
		Printer::printString(confirmingMessage,
				sizeof(confirmingMessage) / sizeof(char));
		char c;
		while (!DRIVER.recv(&c))
			;
		Printer::printChar(c);
...
public:
	UserCommandDto(UserInstruction instruction, char input) :
...
// initialise_monitor_handles();
	...
	while (1) {
		...
		case TOGGLE_IT_MODE:
			session.recordActivity();
			DRIVER.switchMode();
			Printer::printString(modeSwitchedMessage,
					sizeof(modeSwitchedMessage) / sizeof(char));
			if (DRIVER.current == UartDriver::INT) {
				char mode[] = "INTERRUPT\n";
